# slidev-addon-timing-bar

Section-Based Timing for Slidev

Plan your talk. Track your progress. Review your timing.

<div class="my-8 flex justify-center gap-4">
  <a href="https://github.com/c-harding/slidev-addon-timing-bar" target="_blank" class="px-4 py-2 rounded bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 no-underline">
    <carbon-logo-github class="inline mr-1" /> GitHub
  </a>
  <button class="px-4 py-2 rounded bg-blue-500 text-white hover:bg-blue-600" @click="$nav.enterPresenter()">
    <carbon-presentation-file class="inline mr-1" /> Open Presenter Mode (demo)
  </button>
  <Link to="quick-reference" class="px-4 py-2 rounded bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 no-underline">
    <carbon-catalog class="inline mr-1" /> Quick Reference
  </Link>
</div>

_Charlie Harding · April 2026_

<!--
Welcome! This slide deck explains the slidev-addon-timing-bar addon — and dogfoods it at the same time.
The timing bar is live in presenter mode right now.

There are different versions of the slides in this repo:

- <a href="https://c-harding.github.io/slidev-addon-timing-bar/#/presenter" target="_self">slides.md</a> (default)
- <a href="https://c-harding.github.io/slidev-addon-timing-bar/slides-no-timings/#/presenter" target="_self">slides-no-timings.md</a> (no sections or durations at all)
- <a href="https://c-harding.github.io/slidev-addon-timing-bar/slides-with-untimed/#/presenter" target="_self">slides-with-untimed.md</a> (includes untimed sections for testing)
- <a href="https://c-harding.github.io/slidev-addon-timing-bar/slides-total-duration/#/presenter" target="_self">slides-total-duration.md</a> (only total duration set, no sections)
- <a href="https://c-harding.github.io/slidev-addon-timing-bar/slides-total-and-section-durations/#/presenter" target="_self">slides-total-and-section-durations.md</a> (total and section durations set, but no buffer)
-->
